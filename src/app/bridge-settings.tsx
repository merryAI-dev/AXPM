  ...
  return (
    <section className="panel">
      <div className="panelTitle">
        <h2>Hermes · Claude MCP 연결</h2>
        <span className="badge">모델 교체 가능</span>
      </div>
      <p className="hint">
      ...
      {key && (
        <label>
          AXPM_BRIDGE_KEY · 비공개로 보관
          <input readOnly value={key} onFocus={(e) => e.target.select()} />
          <small>
            키는 이 화면에 한 번 표시됩니다. Git 또는 채팅에 붙이지 마세요.
          </small>
        </label>
      )}
      {error && (
      ...
      <p className="hint">
        연결 방법과 스킬: 저장소 docs/HERMES.md · .claude/skills
      </p>
    </section>
  );
}
