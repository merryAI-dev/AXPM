...
const descriptions: Record<keyof typeof toolSchemas, string> = {
  ...
  inspect_company:
    "기업의 완료회차, 원본 셀 근거, 신청 일정과 잔여 티켓 기록을 읽습니다.",
  search_work_mail:
    "사용자가 승인한 업무 검색 범위 안에서 해당 기업의 최근 메일 메타데이터와 미리보기를 읽습니다. 본문 전체를 읽었다고 주장하지 마세요.",
  inspect_calendar:
    "연결 계정의 Google Calendar 일정과 충돌을 확인합니다. 다른 멘토의 전체 캘린더는 알 수 없습니다.",
  propose_action:
    "메일 발송·캘린더 초대·티켓 조정을 제안합니다. 실행은 반드시 UI의 사용자 승인 이후입니다. ticketMode=set은 남은 수량 지정, adjust는 증감입니다. hoursDelta는 시간 단위입니다.",
  save_report_draft:
...
export async function runAgent(uid: string, goal: string, scheduled = false) {
  ...
  const userEvidence = `user:${id}`;
  const extraEvidence: string[] = scheduled ? [] : [userEvidence];
    ...
    const messages: Anthropic.MessageParam[] = [
      {
        ...
        content: JSON.stringify({
          ...
          currentRequest: goal,
          currentRequestEvidence: scheduled ? null : userEvidence,
      ...
      for (const call of calls) {
        ...
        try {
          ...
          if (call.name in workspaceTools)
            ...
            const company = state.snapshot?.companies.find(
              (x) => x.id === input.companyId,
            );
            if (!company?.email || !z.email().safeParse(company.email).success)
              throw new Error("유효한 기업 이메일이 없습니다.");
            const mail = await gmailSearch(
              uid,
              (await settings(uid)).gmailQuery,
              company.email,
            );
            extraEvidence.push(...mail.map((x) => x.id));
            result = mail;
            ...
            if (
              ...
              throw new Error("캘린더 조회 범위는 31일 이내입니다.");
            const events = await calendarList(uid, start, end);
            extraEvidence.push(...events.map((x) => x.id));
            result = events;
          } else if (call.name === "propose_action")
            result = await propose(uid, input, extraEvidence);
          else {
            if (scheduled)
              throw new Error("정기 점검에서는 보고서를 작성하지 않습니다.");
            const parsed = toolSchemas.save_report_draft.parse(input);
            const company = state.snapshot?.companies.find(
              (x) => x.id === parsed.companyId,
            );
            if (!company || parsed.fields.company !== company.name)
              throw new Error("보고서 기업명이 원본과 일치해야 합니다.");
            const allowed = new Set([
              ...(state.snapshot?.evidence.map((x) => x.id) || []),
              ...extraEvidence,
            ]);
            if (
              !parsed.evidenceIds.includes(userEvidence) ||
              parsed.evidenceIds.some((x) => !allowed.has(x))
            )
              throw new Error("이번 대화의 멘토링 메모 근거가 필요합니다.");
            ...
            result = {
              id: reportId,
              message: "보고서 초안 저장. 검토 후 다운로드해주세요.",
            };
          }
        } catch (e) {
          error = true;
