...
export async function inspectTemplate(buffer: Buffer) {
  if (buffer.length > 10_000_000)
    throw new Error("보고서 템플릿은 10MB 이하로 업로드해주세요.");
  const w = new ExcelJS.Workbook();
  await w.xlsx.load(buffer as never);
  if (!w.worksheets.length) throw new Error("템플릿에 시트가 없습니다.");
  return {
    sheets: w.worksheets.map((x) => x.name),
    mapping: defaultMapping,
    imageCount: w.worksheets.reduce((sum, s) => sum + s.getImages().length, 0),
  };
}
...
export async function patchWorkbook(
  ...
  for (const [field, addr] of Object.entries(mapping)) {
    ...
    if (
      ...
      fields[field].length > 32767 ||
      /[\x00-\x08\x0B\x0C\x0E-\x1F]/.test(fields[field])
    )
      throw new Error(`${fieldLabels[field]} 내용이 유효하지 않습니다.`);
  }
  const zip = await JSZip.loadAsync(buffer);
  const workbook = await zip.file("xl/workbook.xml")!.async("string");
  const sheetTag = [...workbook.matchAll(/<sheet\b[^>]*>/g)].find((x) =>
    new RegExp(
      `name="${escapeXml(sheetName).replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}"`,
    ).test(x[0]),
  )?.[0];
  const relId = sheetTag?.match(/r:id="([^"]+)"/)?.[1];
  const rels = await zip.file("xl/_rels/workbook.xml.rels")!.async("string");
  const relation = [...rels.matchAll(/<Relationship\b[^>]*>/g)].find((x) =>
    x[0].includes(`Id="${relId}"`),
  )?.[0];
  ...
  for (const [field, addr] of Object.entries(mapping)) {
    ...
    if (!match)
      throw new Error(`${addr}: 원본에 존재하는 셀만 매핑할 수 있습니다.`);
