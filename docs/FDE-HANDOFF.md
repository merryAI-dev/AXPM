...
[문서 워크스페이스](DOCUMENT-WORKSPACE.md)에 실제 XLSX/Sheets 뷰어 구성, 렌더링 범위, HWPX 로컬 설치·생성·검사 명령과 브라우저 검증 방법을 정리했다. [상시 에이전트 운영](AGENTIC-OPERATIONS.md)은 현재 정기 작업 코드와 변경 감지·사례 관리·승인 후 재확인에 필요한 후속 구현을 구분한다.
