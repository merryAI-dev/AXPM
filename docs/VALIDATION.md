...
## 2026-09-16 · 원본 문서 뷰어와 HWPX 실행 어댑터

- 제공받은 실제 멘토링 보고서 XLSX의 셀 서식·병합·삽입 그림 3개를 브라우저에서 렌더링했다. 합성 수정 사본으로 셀 선택/전체 내용 편집/즉시 미리보기/전체 화면/Escape/미저장 시트 전환 경고를 검증했다.
- 저장·다운로드 후 원본 ZIP 멤버 집합이 일치하며 수정한 시트 XML 하나 외의 모든 파일 payload가 보존됨을 확인했다. 390px 모바일의 페이지 가로 넘침과 브라우저 예외가 없다. 생성한 업로드 사본은 정리했다.
- 렌더러 회귀 테스트 3개를 포함해 25개 단위 테스트가 통과했다. native Sheets 렌더링은 formattedValue·서식·치수·병합·수식 보호의 제공자 데이터 계약을 시험했으며 이 변경에서 실제 Sheets 렌더 화면을 별도로 시험한 것은 아니다.
- 고정 리비전의 hwpx-skill을 로컬 설치하고 AXPM 어댑터로 합성 보고서 HWPX를 실제 생성했다. 필수 문구·ZIP/XML·참조·본문 보존 검사 및 strict 검사가 통과했다. 양식 분석과 실제 본문 텍스트 추출도 확인했다. 한컴 열림·실제 페이지 렌더·육안 검토는 macOS에서 수행하지 않았다. 웹 HWPX 편집이나 Hermes HWPX 실행 도구까지 구현했다는 의미가 아니다.

이 변경의 TypeScript 검사와 Next production build도 통과했다.
